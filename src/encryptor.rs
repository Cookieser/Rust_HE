            ...
            SchemeType::BGV => {
                ...
                if !context_data.qualifiers().using_fast_plain_lift {
                    ...
                    for i in 0..plain_coeff_count {
                        let plain_value = plain.data()[i];
                        if plain_value >= plain_upper_half_threshold {
                            util::add_uint_u64(
                                plain_upper_half_increment,
                                plain_value,
                                &mut temp[coeff_modulus_size * i .. coeff_modulus_size * (i + 1)]
                            );
                        } else {
                    ...
                    plain_copy.data_mut().copy_from_slice(&temp);
                } else {
                    let plain_data = plain_copy.data_mut();
                    for i in (0..coeff_modulus_size).rev() {
                        for j in 0..plain_coeff_count {
                            plain_data[i * coeff_count + j] = if plain_data[j] >= plain_upper_half_threshold {
                                plain_data[j] + plain_upper_half_increment[i]
                            } else {
                                plain_data[j]
                            };
                        }
                ...
                polymod::ntt_p(plain_copy.data_mut(), coeff_count, ntt_tables);
...
impl Decryptor {
    ...
    fn bgv_decrypt(&self, encrypted: &Ciphertext, destination: &mut Plaintext) {
        ...
        polymod::intt_p(&mut tmp_dest_modq, coeff_count, context_data.small_ntt_tables());
