...
/// 
/// 
/// When the scheme is BFV each coefficient of a plaintext is a 64-bit word, but when the scheme is CKKS the plaintext is by default stored in an NTT transformed form with respect to each of the primes in the coefficient modulus. Thus, the size of the allocation that is needed is the
