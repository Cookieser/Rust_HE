...
impl Evaluator {
    ...
    pub fn pack_lwe_ciphertexts(&self, lwes: &[LWECiphertext], automorphism_keys: &GaloisKeys) -> Ciphertext {
        ...
        let ntt_form = context_data.is_bgv() || context_data.is_ckks();
...
mod tests {
    ...
    fn test_pack_lwe_bfv_bgv(use_bgv: bool) {
        // setup 
        let params = EncryptionParameters::new(if use_bgv {crate::SchemeType::BGV} else {crate::SchemeType::BFV})
            .set_plain_modulus_u64(17)
            .set_coeff_modulus(&CoeffModulus::create(32, vec![30, 30, 30]))
            .set_poly_modulus_degree(32);
        let context = HeContext::new(params.clone(), true, crate::SecurityLevel::None);
    ...
    fn test_pack_lwe_bfv() {
        test_pack_lwe_bfv_bgv(false);
    }

    #[test]
    fn test_pack_lwe_bgv() {
        test_pack_lwe_bfv_bgv(true);
    }
